...
for t in test_versioning_logic test_submission_utils test_index_engine test_explorer_agg \
         test_metric_engine test_chain_contract test_coverage test_index_templates test_bulk_parse; do
...
**Immutability (`versioning.py`, shared by survey and index versions).** Once `Published`, a version's only permitted transition is `Published → Closed`, and its **content is frozen too** (not just status — a `Published→Published` save must not rewrite it). `UCC Index Result` rows are immutable snapshots tied to the exact published formula version. The product invariant behind all this: **every published score must be reproducible**. Editing a formula means a new version + new results, never a silent change.
...
- **Mapping**: UCC Objective, UCC Standard, UCC Question Mapping (objective/clause — **1:1 per question**, enforced by a `unique` constraint; verify against the real mapping PDF before importing data — BENCH_VERIFY V6), and UCC Metric Definition → UCC Metric Source (question→metric). Objective mapping and metric mapping are deliberately **separate** records.
