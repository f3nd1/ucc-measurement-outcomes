...
@frappe.whitelist()
def mapping_history(survey_version=None, limit=25):
	"""Who changed an objective mapping, when, and what they changed.

	Round-7 Item 2: the Governance tab used to be one sentence SAYING that
	Frappe records document history, without ever showing any. This reads that
	same history - core's own `Version` records for UCC Question Mapping - so
	the tab shows the audit trail instead of describing it. No new storage: if
	Frappe did not already track it, this returns nothing rather than inventing
	a log.

	`survey_version` is accepted and used to scope to that version's mappings;
	omitted, it reports across every mapping the user can read.
	"""
	_require(survey_version, "read") if survey_version else None
	names = None
	if survey_version:
		questions = frappe.get_all(QUESTION, filters={"survey_version": survey_version}, pluck="name")
		names = frappe.get_all(MAPPING, filters={"question": ["in", questions or [""]]}, pluck="name")
		if not names:
			return []
	filters = {"ref_doctype": MAPPING}
	if names is not None:
		filters["docname"] = ["in", names]
	rows = frappe.get_all(
		"Version", filters=filters, fields=["docname", "owner", "creation", "data"],
		order_by="creation desc", limit=int(limit))
	out = []
	for r in rows:
		changed = []
		try:
			payload = frappe.parse_json(r.get("data") or "{}") or {}
			# Frappe stores edits as [fieldname, old, new] triples.
			for entry in (payload.get("changed") or [])[:4]:
				if isinstance(entry, (list, tuple)) and entry:
					changed.append(str(entry[0]))
		except Exception:
			# A malformed Version row must not take the whole tab down.
			changed = []
		out.append({
			"mapping": r.get("docname"),
			"user": r.get("owner"),
			"on": r.get("creation"),
			"fields": changed,
			"created": not changed,
